...
class Menu():
    def __init__(self, fps, display):
        ...
        self.available_heroes.add(self.menu_cap, self.menu_hulk, self.menu_thor, self.menu_spider)
        self.selected = False
        self.player = None
        ...
        for p in self.available_heroes:
            ...
            if not p.disabled:
                if p.name == "Captain America":
                    ...
                    self.player = Thor(w=160, h=230, x=SCREEN_WIDTH/2, y=550, action='IDLE', hp=100, score=0)
